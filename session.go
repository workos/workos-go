...
// 32 bytes long and is hashed with SHA-256 to derive a key. Shorter passwords
...
// decoded directly. Otherwise, the password must be at least 32 bytes long
// (len counts UTF-8 bytes, not characters) and is hashed with SHA-256 exactly
// as before, so existing cookies sealed under compliant passwords remain valid.
// Empty and short passwords are rejected to guard against session forgery with
// known or easily guessed keys.
func deriveKey(password string) ([]byte, error) {
	...
	if len(password) < 32 {
		return nil, fmt.Errorf("workos: cookie password must be at least 32 bytes")
	}
